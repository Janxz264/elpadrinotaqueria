...
error_reporting(E_ALL);
	...
	if (!$dblink) {
			...
			$sqlquery=$dblink->prepare("INSERT INTO notas VALUES (null,?,?,?)");	
			if($sqlquery->execute(array($fecha,$efectivo,$tarjeta)))
			{
				$last_id = $dblink->lastInsertId();
				for ($i=0; $i < $count/2; $i++) { 
					$sqlquery2=$dblink->prepare("INSERT INTO notas_productos VALUES(?,?,?)");
